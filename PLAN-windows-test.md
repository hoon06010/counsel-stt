...
- [ ] 덮개 닫고도 처리 계속되는지 — **전원 연결 시 / 배터리 사용 시 각각 확인**
  - 2026-07-26 전원 설정은 적용 완료: 덮개 동작 `아무 것도 하지 않음`(AC/DC), 절전 `안 함`(AC/DC),
    무인 절전 `안 함`(AC/DC). 이 기기는 제어판에 덮개 항목이 없어 `powercfg`로 설정
    (`README.md` 3장). 작업 스케줄러 `counsel-stt`의 배터리 관련 조건도 모두 해제 확인.
  - 남은 것은 **실제 동작 확인**: 덮개를 닫은 상태에서 파일을 넣고 `log.txt`에 계속 기록되는지.
    설정값이 맞아도 펌웨어가 무시할 수 있으므로 이 확인은 생략하지 마세요.
